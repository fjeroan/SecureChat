...
def start_private_session(sender, message):
    ...
    try:
        # Encrypt the keys with each client's public key
        ...
        # Send the encrypted keys to both clients
        clients[sender].send(encrypted_for_sender)
        clients[target].send(encrypted_for_target)
        ...
        print(f"Error during key distribution: {e}")
...
def authenticate_client(client_socket):
    ...
    try:
        ...
        # Receive the client's public key for future use
        ...
        print(f"Public key registered for client: {username}")
